...
test('a concurrent check returns the existing projection without another network request', async t => {
  const home = await temporary(t), directory = join(home, 'updates'); await mkdir(directory);
  await atomicJson(join(directory, 'check.json'), { currentVersion: RELEASE_VERSION,
    checkedAt: new Date().toISOString(), nextCheckAt: Date.now() + 600000, policy: policy(), error: null });
  const unlock = await acquireProcessLock(join(directory, '.check-lock.sqlite'));
  let network = 0;
  try {
    const status = await checkForUpdates(home, { force: true, fetcher: async () => { network++; return Response.json(policy()); } });
    assert.equal(status.currentVersion, RELEASE_VERSION);
