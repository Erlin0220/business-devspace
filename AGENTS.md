...
- The 2026-09 recovery migrates the legacy `MASTER_KEY` to network-hosted `MASTER_KEY_V2` through dual-key Worker compatibility: new enrollments use V2, retained ciphertext is decrypted with the old root and re-sealed with V2 under compare-and-swap, and the old Worker secret may be deleted only after every retained ciphertext is proven readable with V2. `MASTER_KEY_V2` then becomes the canonical D1 encryption root in `business-devspace:production` with an independent protected recovery copy.
...
- The active source repository is Erlin0220/business-devspace; the old private repository is an internal history archive, not the integration target. Do not merge its old Git history or copy its Releases/Actions metadata into the public repository.
- The GitHub Windows/Linux/macOS ARM64/native Intel replacement passed one exact-commit four-platform cohort before Codemagic, external Intel handoff and the Rosetta publication waiver were retired. Use the existing native matrix, not a second CI provider; evidence and public-binary restrictions are in docs/public-readiness.md. A build-only candidate is not a publishable release.
- Disable automatic updates only in disposable manual-upgrade fixture homes before executing immutable historical installers. An old updater can otherwise replace sample candidate bytes with a live rollout; never weaken exact-byte verification or change employee update policy to make the fixture pass.
...
- Publication readiness includes remote refs, metadata/logs/artifacts and actual GitHub protections. Local Codex checkpoint refs must not be pushed. Source changes do not authorize repository visibility/history changes, production deployment, binary distribution or version-policy changes. See docs/public-readiness.md.
